...
class Transaction(Base):
    ...
    source_amount = Column(
        Float, nullable=True
    )  # actual amount deducted/received in source currency

    source_currency = Column(
        String(3), nullable=True
    )  # currency of the source amount, if different from asset currency

    balance_after = Column(Float)  # book balance after this transaction

    realized_pnl = Column(
        Float, nullable=True
    )  # realized profit and loss for this transaction
