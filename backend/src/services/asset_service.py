    ...
    def create_asset(db: Session, asset_in: schemas.AssetCreate) -> models.Asset:
        ...
        try:
            ...
            if asset_in.source_asset_id:
                ...
                source_asset = db.query(models.Asset).filter(models.Asset.id == asset_in.source_asset_id).first()
                if not source_asset:
                    raise HTTPException(status_code=404, detail="Source asset not found")
                
                # Calculate deduction amount
                # Use source_amount if provided (e.g. TWD cost), otherwise default to initial cost (if same currency)
                deduct_amount = asset_in.source_amount if asset_in.source_amount is not None else asset_in.initial_total_cost
