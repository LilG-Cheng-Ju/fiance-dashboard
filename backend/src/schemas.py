...
class TransactionCreate(BaseModel):
    ...
    amount: float = Field(
        ..., description="Total amount change (negative for buy, positive for sell)"
    )
    quantity_change: float = Field(
        0.0, description="Quantity change (positive for buy, negative for sell)"
    )
    ...
    source_amount: Optional[float] = Field(
        None, description="Actual deducted amount in source currency"
    )
    source_currency: Optional[str] = Field(
        None, description="Source currency code (e.g., TWD)"
    )
    source_asset_id: Optional[int] = Field(
        None, description="ID of the asset to deduct funds from (for BUY/DEPOSIT)"
    )
...
class AssetCreate(BaseModel):
    ...
    initial_total_cost: float = Field(
        0.0, description="Initial total cost / initial balance"
    )
    initial_quantity: float = Field(0.0, description="Initial quantity")

    source_asset_id: Optional[int] = Field(
        None, description="ID of the asset used to pay for this new asset"
    )

    source_amount: Optional[float] = Field(
        None, description="Actual initial deducted amount"
    )
    source_currency: Optional[str] = Field(
        None, description="Initial source currency code"
    )
    exchange_rate: Optional[float] = Field(
        1.0, description="Exchange rate at the time of creation"
    )
